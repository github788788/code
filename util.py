...
for a,val in enumerate(__list__):
    print(val)
...
next_stage =str(input("click to continue/commence github"))
...
def gen_emails(inp):
	#emails = gen_emails([])
	emails = []
	emails.append("media788788@gmail.com")
	#emails.append("desk788788@gmail.com")
	emails.append("violin78@protonmail.com")
	emails.append("cello34@protonmail.com")
	emails.append("violin78@mail.com")
	emails.append("usa2@email.com")
	text = ""
	for a,val in enumerate(emails):
		text = text+val+","
	write_data(["gen_emails.txt",text])
	return text
...
def save_array_as_xls(inputs):
    #save_array_as_xls(inputs)
    array = inputs[0]
    file_path = inputs[1]
...
def write_data(inp):
	#write_data([fil,dat])
	#write_data([fil,ray])
	#write_data([output_file,earnings_dates])
	fil = inp[0]
	out = inp[1]
	#print("wh")
	#print(out)
	...
	if ".xls" in fil:
		...
		new_xls.save(fil)
	if ".csv" in fil: 
		#new_file = file.replace(".csv","2.csv") 
		file = open(fil, 'w+', newline ='')
		with file:    
...
"""
open with opera instead of notepad/wordpad
and then just do ctrl+a and then ctrl f4
and the paste it..no need to open in other
...
def copy_paste(inp):
	...
	output_file = "copy_paste.txt"
	write_data([output_file,text_to_copy_paste])
...
def single_url(inp):
	...
	for a,val in enumerate(urls):
		...
		text = val+"\n"
		wri2([save,text])
		subprocess.Popen(['notepad.exe', save])
		time.sleep(1)
		#sw2([save,1])
		#hod3(["shift","down",1,0])
		hod3(["ctrl","a",1,0])
...
def multiply_by_10s(inp):
	#multiply_by_10s([number])
	number = inp[0]
	number = str(inp[0])
	extensions = []
	extensions.append(["K",1000])
	extensions.append(["M",1000000])
	extensions.append(["B",1000000000])
	#new_number = "could not return"
	new_number = number
	for a,val in enumerate(extensions):
		identifier = val[0]
		if identifier in number:
			new_number = number.replace(identifier,"")
			new_number = float(new_number)
			new_number = new_number*val[1]
			break
	return new_number
...
def opa4(inp):
	...
	if "opera" in browser or "Opera" in browser:
		...
		hod3(["ctrl","f4",1,1])
		hod3(["ctrl","f4",1,1])
	if "firefox" in browser or "Firefox" in browser:
		#cf_ee("open")	
		cf_ete2(["open",3])
		hod3(["ctrl","pageup",1,2])
		hod3(["ctrl","f4",1,0])
...
def list_to_dataframe(inp):
	#array = list_to_dataframe([list])
	#array = list_to_dataframe([list])
	#import numpy 
	import numpy as np 
	import pandas as pd 	
...
def click_logo(inp):
	...
	#logo_name = "logo" 
	#file_name = "logo"
	if "." not in logo_location:
		logo_location =logo_name+".png"
...
def click_logo3(inp):
	#click_logo3(["new_message.png",2])
	logo_name = inp[0]
	wait = inp[1]
	location = pyautogui.locateOnScreen(logo_name)	
	#print(location)
	pyautogui.moveTo(location[0],location[1])
	pyautogui.click()
	time.sleep(wait)
...
line_number = getframeinfo(current).lineno
display = "stopping code on line "+str(line_number)
print(display)
"""
from tkinter import *
...
def gen_gui(inputs):
	...
	what_to_generate = inputs[0]
	import tkinter
	gui = tkinter.Tk()
	gui.title("give gui a name")
	gui.geometry("250x300")
	#new_name = ""
	for a,val in enumerate(what_to_generate):
		#
		if "title" in val[0] or "Title" in val[0]:
			gui.title(val[1])
			break
	for a,val in enumerate(what_to_generate):
		#
		if "geometry" in val[0] or "Geometry" in val[0]:
			gui.geometry(val[1])
			break
	row = -1
	new_file = ""
	new_file = new_file+"import tkinter\ngui = tkinter.Tk()\n"
	new_file = new_file+"gui.title(\""+str(gui.title())+"\")\n"
	#new_file = new_file+"gui.geometry(\""+str(gui.geometry())+"\")\n"
	for a,val in enumerate(what_to_generate):
		print(val)
		column = 0
		row = row+1
		type_of_object = val[0]
		text = val[1]
		if "Entry" in type_of_object or "entry" in type_of_object:
			new_file=new_file+"Label(gui, text=\""+str(text)+"\").grid(row="+str(row)+",column="+str(column)+")\n"
			new_file= new_file+"new_entry = Entry(gui)\n"
			new_file= new_file+"new_entry.grid(row="+str(row)+",column="+str(column+1)+")\n"
			if len(val)==2:
				#new_entry.insert(0,text)
				#new_file = new+file+ new_entry.insert(0,text)
				new_file = new_file+"new_entry.insert(0,\""+str(text)+"\")\n"
			if len(val)==3:
				#new_entry.insert(0,val[2])
				new_file = new_file+"new_entry.insert(0,\""+str(val[2])+"\")\n"
		if "Button" in type_of_object or "button" in type_of_object:
			#how_to_invoke_function = "command= lambda:stock([what_to_run])"
			how_to_invoke_function = "command= lambda:stock([what_to_run])"
			new_file = new_file+"button_run_file = Button(gui,text =\""+str(text)+"\","+how_to_invoke_function+")\n"
			#"command = "+str(val[2])+")\n"
			if "(" in val[2]:
				file_name = val[2][0:val[2].find("(")]
			if "(" not in val[2]:
				file_name = val[2]
			function_text = load_data([file_name+".py"])
			new_file = function_text+"\n"+new_file
			new_file = new_file+"button_run_file.grid(row="+str(row)+",column="+str(column)+")\n"
			#command= "lambda:\nstock([what_to_run])

	new_file = new_file+"mainloop()"
	print(new_file)
	save_file = "gen_gui.py"
	wri2([save_file,new_file])
	command = "python "+save_file
...
def gen_gui2(inputs):
	...
	for a,val in enumerate(what_to_generate):
		...
		type_of_object = val[0]
		text = val[1]
		if "Label" in type_of_object or "label" in type_of_object:
			Label(gui, text=text).grid(row=row)
		if "Entry" in type_of_object or "entry" in type_of_object:
			Label(gui, text=text).grid(row=row,column=column)
			new_entry = Entry(gui)
			column = column+1
			...
			if len(val)==3:
				new_entry.insert(0,val[2])
		if "Function" in type_of_object or "function" in type_of_object:
			python_file = str(val[1])
			if ".py" not in val:
				python_file = python_file+".py"
			exec(open(python_file).read())
			print(python_file)
		if "File" in type_of_object or "file" in type_of_object:
			python_file = str(val[1])
			if ".py" not in val:
				python_file = python_file+".py"
			exec(open(python_file).read())
		...
		if "Button" in type_of_object or "button" in type_of_object:
			if len(val)>2:	
				button_run_file = Button(gui,text =text,command = val[2])
			button_run_file = Button(gui,text =text)
			button_run_file.grid(row=row,column=column)
	#sye()
	mainloop()

def gen_gui3(inputs):
	#gen_gui3([gui_commands])
	to_do = inputs[0]
	for a,val in enumerate(to_do):
		val[0] = val[0].lower()
	final_code = ""
	final_code = final_code+"exec(open('util.py').read())"
	...
	for a,val in enumerate(to_do):
		...
		if "entry" in val0:				
			...
			val3 = val[3]
			row=row+1
			final_code=final_code+"\n\tLabel(run, text=\""+val1+"\").grid(row="+str(row)+",column="+str(column)+")"
			final_code=final_code+"\n\tdef shell_entry"+str(row)+"():"
			final_code=final_code+"\n\t\tfile"+str(row)+"=new_entry"+str(row)+".get()" 
			final_code=final_code+"\n\t\tshell([file"+str(row)+"])"  
			final_code= final_code+"\n\tnew_entry"+str(row)+" = Entry(run)"
			final_code= final_code+"\n\tnew_entry"+str(row)+".grid(row="+str(row)+",column="+str(column+1)+")"
			python_file_to_load = "shell.py"
			if python_file_to_load not in already_functions:
				function_text = load_function([python_file_to_load])
				#function_text = function_text.replace("exec(open('util.py').read())\n","")
				final_code = function_text+"\n"+final_code+"\n"
				already_functions.append(python_file_to_load)
			final_code = final_code+"\n\tbutton_run_file = Button(run,text =\""+val2+"\",command = "+val3+")"
			#final_code = final_code+"\n\tbutton_run_file = Button(run,text =\""+val2+"\",command = "+val3+")"
			final_code = final_code+"\n\tbutton_run_file.grid(row="+str(row)+",column = "+str(column+2)+")"
		if "button" in val0:
			#gui_list.append(["button","server2","server2.py"])
			val2 = val[2]
			row=row+1
			python_file_to_load = val2
			if python_file_to_load not in already_functions:
				#loaded_file = load_data([python_file_to_load])
				function_text = load_function([python_file_to_load])
				#function_text = function_text.replace("exec(open('util.py').read())\n","")
				final_code = function_text+"\n"+final_code+"\n"
				already_functions.append(python_file_to_load)
			function_to_run = python_file_to_load.replace(".py","")
			final_code = final_code+"\n\tbutton_run_file = Button(run,text =\""+val1+"\",command = "+function_to_run+")"
			final_code = final_code+"\n\tbutton_run_file.grid(row="+str(row)+",column = "+str(column+2)+")"
	final_code = final_code+"\n\tmainloop()"
	final_code = final_code+"\ninp = []"
	final_code = final_code+"\nrun_gui(inp)"
	print(final_code)
	save_file = "run_gui.py"
	write_data([save_file,final_code])
	start_file([save_file,0])
	#sye()
	command = "python "+save_file
	os.system(command)

def load_function(inputs):
	#function_to_run = load_function(["shell.py"])
	file_to_load_from = inputs[0]
	loaded_file = load_data([file_to_load_from])
	function_name = file_to_load_from.replace(".py","")
	replace2 = "inp = []\n"+function_name+"(inp)"
	loaded_function = loaded_file.replace(replace2,"")
	#loaded_function = loaded_function.replace("inp = []","")
	print(loaded_function)
	out_file = function_name+"_function.py"
	write_data([out_file,loaded_function])
	#exec(open(out_file).read())
	return(loaded_function)

def gen_html_table(inputs):
	# html_code = gen_html_table([array])
	array = inputs[0]
	html_code = "<table>"
	for a,val in enumerate(array):
		html_code = html_code+"<tr>"
		for b,valb in enumerate(val):
			html_code = html_code+"<td>"+str(valb)+"</td>"
		html_code = html_code+"</tr>"
...
def get_current_partition():
    ...
    for partition in psutil.disk_partitions(all=True):
        if current_directory.startswith(partition.mountpoint):
            return partition.device
    return None


def get_partitions_by_size():
    partitions = []
    for partition in psutil.disk_partitions(all=True):
        try:
            partition_usage = psutil.disk_usage(partition.mountpoint)
            partitions.append((partition.mountpoint, partition_usage.total))
        except PermissionError:
            # Handle PermissionError if the user does not have permission to access the partition
            pass
