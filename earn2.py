...
def ear5(inp):
	...
	#for a,val in enumerate(ray[0:10]):
	for a,val in enumerate(ray[0:5]):
		if a==len(ray)-1:
			break
		ide1 = val+", "
	...
	#pri(mas4[0:20])
	#sw(nam,1)
	#line where to specify how many stocks
	#list_length
	#lis = mas4[0:how_many]
	lis = mas4[0:list_length]
	sto = lis
	#sto = mas4[0:20]
	sto2 = []
	...
	mis = che([lise,nee])
	#print(mis)
	#alpha vantage query limit?
	key2 = "FK98MBU38O64HAMH"
	for a,val in enumerate(mis):
		sym = val.replace(".json","")
		url = "https://www.alphavantage.co/query?function=EARNINGS&symbol="+sym+ "&apikey="+key2
		print(a,len(mis),sym)
		print(url)
